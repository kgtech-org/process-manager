...
type Macro struct {
	...
	DomainID         *primitive.ObjectID `json:"domainId,omitempty" bson:"domain_id,omitempty"` // Link to Domain
...
type CreateMacroRequest struct {
	...
	DomainID         *string `json:"domainId"`                            // Optional: Link to domain
}
...
type MacroFilter struct {
	...
	DomainID  *string `json:"domainId"` // Filter by domain
	Page      int     `json:"page"`
	Limit     int     `json:"limit"`
	IsActive  *bool   `json:"isActive"` // Filter by active status
	SortBy    string  `json:"sortBy"`   // name, created_at, updated_at
	SortOrder string  `json:"order"`    // asc, desc
}
